...
import org.loon.framework.android.game.core.geom.Point.Point2i;
import org.loon.framework.android.game.core.geom.RectBox.Rect2i;
...
import org.loon.framework.android.game.utils.xml.XMLDocument;
...
public class LTexturePack implements LRelease {
	...
	public Point2i draw(int id, float dx1, float dy1, float dx2, float dy2,
		...
		return draw(id, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2, 0, null);
	}
		...
		if (GLEx.self != null) {
			...
			if (GLEx.self.useGLBegin() && rotation == 0 && color == null) {
				GLEx.self.draw(texture, dx1, dy1, dx2, dy2, sx1
						+ entry.bounds.left, sx2 + entry.bounds.top, sy1
						+ entry.bounds.right, sy2 + entry.bounds.bottom, false);
			} else {
				GLEx.self.drawTexture(texture, dx1, dy1, dx2, dy2, sx1
						+ entry.bounds.left, sx2 + entry.bounds.top, sy1
						+ entry.bounds.right, sy2 + entry.bounds.bottom,
						rotation, color);
			}
			blittedSize.set(entry.bounds.width(), entry.bounds.height());
		}

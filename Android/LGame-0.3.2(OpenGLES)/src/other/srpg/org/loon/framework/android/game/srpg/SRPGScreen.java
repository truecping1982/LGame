...
public abstract class SRPGScreen extends Screen implements SRPGType, Runnable {
		...
		for (int i = 0; i < srpgActors.size(); i++) {
			...
			if (result != null) {
				...
				g.drawStyleString(result, (actor.drawX() + size) - cam_x, actor
						.drawY()
						- 3 - cam_y, GLColor.red, GLColor.white);
				g.setFont(old);
			}
		}
	 ...
	 * @param y
	 */
	private final void drawCubeView(GLEx g, GLColor color, int x, int y) {
	...
	public void setHeroMoveColor(GLColor c) {
		this.moving_our = c;
	}

	public void setEnemyMoveColor(GLColor c) {
		this.moving_other = c;
	}

	public void setAttackTargetColor(GLColor c) {
		this.attack_target = c;
	}

	public void setAttackRangeColor(GLColor c) {
		this.attack_range = c;
	}

	public void setVectorChangeColor(GLColor c) {
	...
	public final WaitAnimation getDefWaitAnimation() {
		if (waitDraw == null) {
			waitDraw = new WaitAnimation(1, getWidth(), getHeight());
		}
		return waitDraw;
	...
	public final LTimer getDefTimer() {
		if (defTimer == null) {
			defTimer = new LTimer(100);
		}
		return defTimer;
	}
